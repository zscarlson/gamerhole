...
exports.BattleFormats = {
	  ...
	  onValidateTeam: function (team) {
	    ...
	    for (let i = 0; i < team.length; i++) {
	      ...
	      if (item.megaStone && template.species === item.megaEvolves) {
	        ...
	        console.log(team[i].baseSpecies);
	    }
    ...
    name: 'Test Clause',
    onStart: function () {
        this.add('rule', 'Weight Clause: chunky boys only');
    },
    onValidateTeam: function (team, format) {
        for (let i = 0; i < team.length; i++) {
            let template = this.getTemplate(team[i].species);
            return [template.species + " " + template.heightm + " " + template.weightkg + " " + template.item];
        }
    },
	},
	tinyclause: {
    ...
    name: 'Tiny Clause',
    onStart: function () {
        this.add('rule', 'tiny tourney Rules: Pok\u00E9mon must be 1 meter tall or less');
    },
    onValidateTeam: function (team, format) {
        for (let i = 0; i < team.length; i++) {
            let template = this.getTemplate(team[i].species);
            if (template.heightm > 1) {
                return ["Pokemon must all be 1 m in height or shorter."];
            }
        }
    },
	},
